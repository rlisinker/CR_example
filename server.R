...
# click information
  output$info <- renderPrint({
    # something is still wrong because i had to take out the facet option in the scatterplot, if i leave it in, it only registers for left most panel
    rows <- nearPoints(space, input$scatter_click, xvar = input$xvar2, yvar = input$yvar, 
                       #panelvar1 = input$facet, 
                       threshold = 5, maxpoints = 3, allRows = TRUE, addDist = T)
    distances = rows$dist_
    indices = which(distances < 5)
    first_subset <- rows[indices, c(1, 15:18, 22)]
    SecondSub <- first_subset[order(first_subset$dist_),]
    SecondSub[1:3, c("Name", input$xvar2, input$yvar)]
  })
